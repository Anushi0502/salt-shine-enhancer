...
const Collections = () => {
  return (
    <section className="salt-container my-12 animate-rise animate-rise-d1">
      ...
      <div className="grid grid-cols-4 gap-3.5 max-lg:grid-cols-2 max-sm:grid-cols-1">
        {collections.map((c) => (
          <a
            key={c.name}
            href="#"
            className="group relative rounded-lg min-h-[210px] border border-salt-line overflow-hidden shadow-soft flex items-end p-4 isolate"
          >
            <div className="absolute inset-0 bg-gradient-to-t from-[rgba(0,0,0,0.65)] to-[rgba(0,0,0,0.2)] z-[-1]" />
            <img
              src={c.img}
              alt={c.name}
              className="absolute inset-0 w-full h-full object-cover z-[-2] transition-transform duration-500 group-hover:scale-[1.08]"
              loading="lazy"
            />
            <div>

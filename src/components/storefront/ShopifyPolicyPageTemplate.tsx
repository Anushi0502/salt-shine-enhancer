    ...
    <section className="mx-auto mt-8 w-[min(1100px,94vw)] pb-8">
      <Reveal>
        <div className="salt-surface-strong rounded-[2rem] p-6 sm:p-8">
          ...
          <p className="mt-2 text-xs text-muted-foreground">Live content fetched from Shopify policies.</p>

          <article
            className="prose prose-sm mt-6 max-w-none rounded-2xl border border-border/70 bg-background/78 p-5 leading-7 text-foreground dark:prose-invert prose-headings:font-display prose-headings:text-foreground prose-a:text-primary prose-strong:text-foreground prose-li:text-foreground prose-p:text-foreground prose-table:block prose-table:w-full prose-table:overflow-x-auto prose-table:border prose-table:border-border prose-th:border prose-th:border-border prose-th:bg-muted/40 prose-th:px-3 prose-th:py-2 prose-td:border prose-td:border-border prose-td:px-3 prose-td:py-2"
            dangerouslySetInnerHTML={{ __html: bodyHtml }}
          />

          <div className="salt-section-shell mt-6 flex flex-wrap gap-2 rounded-2xl border border-border/70 bg-background/78 p-3">
            {actions.map((action) => (
              <Link
                key={action.to}
                to={action.to}
